...
class TestCard(TestCase):
    ...
    def test_init_valid_value(self):
        """checks if the value the user gives enters as the card value"""
        self.assertEqual(self.my_card.value, 2)

    def test_init_valid_symbol(self):
        """checks if the symbol the user gives enters as the card symbol"""
        self.assertEqual(self.my_card.symbol, 1)

    def test_init_invalid_value_type(self):
        """tests for an error to be given when giving a non-int type of value"""
    ...
    def test_init_invalid_value_negative(self):
        """tests for a default value to be given when entering a negative value as the user."""
    ...
    def test_init_invalid_symbol_negative(self):
        """tests for a default value when giving a negative value in the symbol"""
        invalid_value_card = Card(1, -1)
        self.assertEqual(invalid_value_card.symbol, 1)

    def test_gt_valid_higher_value(self):
        """ensures the greater method returns true when the value is higher"""
        higher_card = Card(10,1)
        self.assertGreater(higher_card, self.my_card)

    def test_gt_valid_higher_symbol(self):
        """tests if the greater method returns true when the values are equal but the symbol is higher."""
        higher_card = Card(2,4)
        self.assertGreater(higher_card, self.my_card)

    def test_gt_valid_ace_highest(self):
        """Testing if Ace is the highest card by comparing it to the king card."""
        king_card = Card(13,1)
        ace_card = Card(1,1)
        self.assertGreater(ace_card, king_card)

    def test_gt_invalid_value(self):
        """Testing if the greater method returns false when the card is not higher."""
        other_card = Card(5, 1)
        self.assertFalse(self.my_card > other_card)

    def test_gt_invalid_equal(self):
        """Testing if the greater method returns false when the cards are equal"""
        other_card = self.my_card
        self.assertFalse(self.my_card > other_card)

    def test_gt_invalid_symbol(self):
        """Testing if the greater method returns false when the value is identical and the symbol is lower"""
        other_card = Card(2, 4)
        self.assertFalse(self.my_card > other_card)
    ...
    def test_eq_invalid_value(self):
        """Testing if the equal method will return false when both values are different"""
        other_card = Card(3,1)
        self.assertFalse(self.my_card == other_card)
