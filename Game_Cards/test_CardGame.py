...
class TestCardGame(TestCase):
    ...
    def test_init_invalid_name_two_value(self):
        """Testing for a value error to be raised if the second
        player name is empty"""
        with self.assertRaises(ValueError):
            CardGame("Raz", "")

    def test_init_invalid_name_one_type(self):
        """Testing if an error occurs if the first player name
    ...
    def test_new_game_invalid_called_from_outside_init(self):
        """Testing if the new_game method can be called from out
        of the init method."""
        ...
        self.game1.new_game()
        self.assertTrue(deck_cards == self.game1.game_cards_deck.cards and
                        player1_cards == self.game1.player_one.player_cards and
                        player2_cards == self.game1.player_two.player_cards)

    def test_new_game_valid_player_1_cards_amount(self):
        """Tests if player1 got his cards after new game
        method being called"""
        self.assertEqual(len(self.game1.player_one.player_cards),
                         self.game1.player_one.cards_number)

    def test_new_game_valid_player_2_cards_amount(self):
        """Tests if player2 got his cards after new game
        method being called"""
    ...
    def test_get_winner_valid_player1_loses(self):
        """Tests if player2 wins when he has more cards than player1"""
        self.game1.player_one.get_card()
        self.assertEqual(self.game1.player_two, self.game1.get_winner())

    def test_get_winner_valid_draw(self):
        """Tests if method returns false if both cards
        has the same amount"""
