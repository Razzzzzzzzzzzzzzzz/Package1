...
class Player:
    def __init__(self, name, cards_number = 26):
        ...
        self.name = name
        if 10 <= cards_number <= 26: # Checks for the cards number to be within the allowed range.
    ...
    def set_hand(self, cards_deck = DeckOfCards()):
        """Gives the player a deck of cards according to the cards_number"""
        ...
        if len(cards_deck.cards) > 0: # if there are cards in the cards deck
            for cards in range(self.cards_number):
                self.player_cards.append(cards_deck.deal_one()) #adds a card from the cards deck to the player
