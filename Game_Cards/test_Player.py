...
class TestPlayer(TestCase):
    ...
    def test_set_hand_valid_type(self, mock_deal_one):
        ...
        cards_deck = DeckOfCards()
        self.player1.set_hand(cards_deck)
        self.assertEqual(type(self.player1.player_cards[0]),
                         type(cards_deck.cards[0]))

    def test_set_hand_invalid_empty_deck(self):
        """To ensure the set hand method doesn't give an error when
        the cards deck is empty"""
        cards_deck = DeckOfCards()
        cards_deck.cards = []
        self.player1.set_hand(cards_deck)
    ...
    def test_get_hand_valid(self):
        ...
        being used"""
        self.player1.set_hand() # For the player to have a cards deck
                                # to take a card from
