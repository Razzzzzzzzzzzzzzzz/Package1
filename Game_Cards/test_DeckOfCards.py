...
class TestDeckOfCards(TestCase):
    ...
    def test_init_valid_no_duplicates(self):
        ...
        for card1 in range(52): # The entire cards deck
            for card2 in range(card1 + 1,52): # The cards forward from the current card.
                if card1 == card2: # If a card is found identical to another card in the deck
                    no_duplicates = False # shuffle method worked if the line above is true
    ...
    def test_valid_shuffle(self):
        ...
        for i in range(52):
            if shuffled_cards_deck.cards[i] != self.my_cards_deck.cards[i]:
                shuffled = True # If two cards on the same index are different, the cards deck is shuffled.
                break
    ...
    def test_deal_one_invalid_empty_cards_deck(self):
        """Testing the deal one method to not do anything when there are no cards"""
        self.my_cards_deck.cards = []
        other_cards_deck = self.my_cards_deck.cards.copy()
        self.my_cards_deck.deal_one() # use the deal one on one of the cards deck
